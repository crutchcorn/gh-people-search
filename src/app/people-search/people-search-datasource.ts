    ...
    return merge(
      ...
      if (typeof eventVal === 'string') {
        ...
        if (eventVal === this.searchSubj.value) {
          return observableOf(this.currentServerData.slice(this.paginator.pageIndex, this.paginator.pageSize));
        }
        return this.gitHubService.searchUsers(this.searchSubj.value, 0, {order: this.sort.direction})
          .pipe(map(({total_count, items}) => {
            ...
            return items.slice(0, this.paginator.pageSize);
          }));
      }
